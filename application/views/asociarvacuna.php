<div class="col-xs-12">
                             ...
                             <div class="col-md-6" id="divVolumen">
                                <div class="form-group">
                                  ...
                                  <!--      <select required="" class="form-control" name="cboVolumen" id="cboVolumen">
                                        ...
                                        <input type="text" maxlength="4" class="form-control" id="cboVolumen" name="cboVolumen"  value="" onkeypress="return soloNumeros(event)">
                                    </div>
                                </div>
                            </div> 
          ...
          <section>
                <div class="panel panel-danger">
                    <div class="panel-heading">
                        <h3 class="panel-title">Lista de Vacunas/Desparasitadores con indicaciones</h3>
                    </div>
	...
	$(document).ready(function() {
            ...
            $("#cboEdadMinima").change(function (){
               
                var min = $("#cboEdadMinima").val();
                var max = $("#cboEdadMaxima").val();
              //  alert('min'+min+'-'+max) 
                if(max != 0){
                    if(min > max){
             ...
             $("#cboEdadMaxima").change(function (){
               ...
               // alert('max'+min+'-'+max) 
                if(min != 0){
                    if(max < min){
                        alert('La edad máxima no puede ser menor a la edad mínima');
                        $("#cboEdadMaxima").val('0')
                    }
                }
            });
            ...
            $.ajax({        
                                ...
                                success: function(data) {
                                    ...
                                    if(data == '1'){
                                        ...
                                        alert('No se puede eliminar el registro, cuenta con pautas asignadas');
                                    }else{
                                        alert('No se puede eliminar el registro, se encuentra asiganado a un calendario');
                                    }
...
function listaAsociarVacuna(inicio,fin,id){
        ...
        var pag = (inicio/10)+1;
                                ...
                                success: function(data) {
                                 ...
                                 //   alert(data.total)
                                    ...
                                    $.each( data.registros, function( key, value ) {
                                        ...
                                        if(value["flag_calendario"] == 0){
                                        tbl += "<td><a href='#' onclick=findVac('"+value["id"]+"') data-toggle=\"tooltip\" title=\"Editar\" ><span class=\"glyphicon glyphicon-pencil\" >&nbsp;</span></a>";
                                        tbl += "<a href='#' onclick=confirmaEliminarCal('"+value["id"]+"') ><span style=\"color: black;\" class=\"glyphicon glyphicon-trash\" data-toggle=\"tooltip\" title=\"Eliminar\">&nbsp;</span></a>";
                                        tbl += "<a href='#' onclick=addPau('"+value["id"]+"')><span style=\"color: green;\" class=\"glyphicon glyphicon-circle-arrow-up\" data-toggle=\"tooltip\" title=\"Agregar pauta\"></span></a></td>";
                                           ...
                                           tbl += "<td>Asignado a Calendario(s)</td>" ;
                                        }
                                        tbl += "</tr>";
                                      });
                                      tbl +=" </tr></tbody>  ";  
         ...
         function paginat(page,total){
           if(page == 0){
               ...
               listaAsociarVacuna((ultI-1)*10,10);
            }else{
                listaAsociarVacuna((page-1)*10,10);
            }
            
        }
        
        function addPau(id){
            callMODAL_Bootstrap_formularios("my_modal_addPauta", "Agregar Pautas", '<iframe src="<?php echo getUrl(); ?>/main/addpauta/&id='+id+'" width="1100" height="550" scrolling="yes" frameBorder="0" ></iframe>');
        }
        function callMODAL_Bootstrap_formularios(id_modal,titulo, contenido) {
            ...
            function validar(flag){
               ...
               // alert(txtCalendario.length+"-"+cboEspecie+"-"+cboTipoCalendario+"-"+txtFechaInicio.length+"-"+txtFechaFin.length)
                ...
                if(flag == '1'){
                    ...
                    if( cboVolumen == '' ) {  $("#divVolumen").addClass('has-error') ; retorno += 1 }else{  $("#divVolumen").removeClass('has-error');  retorno += 0 ;}
            ...
            callMODAL_Bootstrap_EditaCalendario("my_modal_editaCal", "Confirmación", '<h5>¿Desea guardar los cambios realizados?</h5>');
           }
        
            function callMODAL_Bootstrap_EditaCalendario(id_modal,titulo, contenido) {
            $("#myModal_total_formularios").html('');
                var modal = '';
                modal += '<div class="modal fade" id="'+id_modal+'\" data-backdrop="static"  data-keyboard="false"  tabindex=\"-1\" role=\"dialog\" aria-labelledby=\"myModalLabel\">';
                modal += '<div class="modal-dialog\" style=\"width:350px;height:400px;\" role=\"document\">';
            ...
            function editarCal(){
                ...
                data:  {cboEspecie:cboEspecie,cboGrpoFarmaco:cboGrpoFarmaco,cboFarmaco:cboFarmaco,cboEdadMinima:cboEdadMinima,cboEdadMaxima:cboEdadMaxima,
                        cboAplicacion:cboAplicacion,cboVolumen:cboVolumen,cboUndMedida:cboUndMedida,efectos:efectos,idHidden:idHidden},
                dataType: "json",
                success: function(data) {
                    ...
                    if(data == '1'){
                        ...
                        alert('No se puede guardar el registro, ya existe una indicación igual');
                        }
                }
            });
            }
           ...
           function confirmaEliminarCal(id){
            callMODAL_Bootstrap_Eliminar("my_modal_elimina", "Confirmación", '<h5>¿Está seguro de eliminar el registro seleccionado?</h5>',id);
           }
        
            function callMODAL_Bootstrap_Eliminar(id_modal,titulo, contenido,id) {
            $("#myModal_total_formularios").html('');
                var modal = '';
                modal += '<div class="modal fade" id="'+id_modal+'\" data-backdrop="static"  data-keyboard="false"  tabindex=\"-1\" role=\"dialog\" aria-labelledby=\"myModalLabel\">';
                modal += '<div class="modal-dialog\" style=\"width:350px;height:400px;\" role=\"document\">';
                ...
                modal += '</div>';
                modal += '<div class=\"modal-footer\">';
                modal += '<button type=\"button\" class=\"btn btn-primary\" id=\"modal_aceptar\" name=\"modal_aceptar\" onclick="eliminarVac('+id+')" data-dismiss=\"modal\">Eliminar</button>';
                modal += '<button type=\"button\" class=\"btn btn-primary\" id=\"modal_aceptar\" name=\"modal_aceptar\" onclick="cerrarModal()" data-dismiss=\"modal\">Cancelar</button>';
                modal += '</div>';
                modal += '</div>';
                modal += '</div>';
